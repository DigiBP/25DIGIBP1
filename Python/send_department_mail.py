...
def build_mail(data: dict) -> EmailMessage:
   ...
   plain = [
       "Hallo zusammen",
       "",
       "Ein Feedback braucht eure Hilfe. Mit folgendem link kannst du das Feedback dokumentieren",
       f"\"{initial}\"",
       "",
   ]
   ...
   msg["From"]    = EMAIL_ADDR
   msg["To"]      = EMAIL_ADDR # sends to digipro email for testing

   # !TODO make department email dynamic
...
def send_email(data: dict):
   ...
   print("E-mail sent to", EMAIL_ADDR)


# ── Worker loop ──────────────────────────────────────────────────────────────
if __name__ == "__main__":
   print("Documentation-mail worker started — polling Camunda …")
   while True:
       try:
